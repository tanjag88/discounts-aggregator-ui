...
test("if click like product = updateProduct  and put correct userId in likes", () => {
  ...
  expect(mutateAsync.mock.calls[0][0].likes).toEqual(["88"]);
});
...
test("if user for the first time is opening the product detail page, check if update Data(userData) and mutate(update product) are called correctly ", () => {
  const state = {
    userId: "10",
    viewedProducts: [],
    likedProducts: [],
